...
void calculateFCFS(std::vector<Process>& processes, std::vector<Process>& timeline) {
    int n = processes.size();
    if (n == 0) return;

    // Work on a copy sorted by arrival time
    std::vector<Process> sortedProcesses = processes;
    std::sort(sortedProcesses.begin(), sortedProcesses.end(), compareArrival);
    ...
    for (int i = 0; i < n; i++) {
        if (currentTime < sortedProcesses[i].arrivalTime) {
            ...
            idle.burstTime = sortedProcesses[i].arrivalTime - currentTime;
            idle.completionTime = sortedProcesses[i].arrivalTime;
            timeline.push_back(idle);
            currentTime = sortedProcesses[i].arrivalTime;
        }

        sortedProcesses[i].completionTime = currentTime + sortedProcesses[i].burstTime;
        ...
        sortedProcesses[i].responseTime = currentTime - sortedProcesses[i].arrivalTime;
        
        timeline.push_back(sortedProcesses[i]);
        currentTime = sortedProcesses[i].completionTime;
    }

    // Write back computed times to original order processes
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (sortedProcesses[j].pid == processes[i].pid) {
                processes[i].completionTime = sortedProcesses[j].completionTime;
                processes[i].turnaroundTime = sortedProcesses[j].turnaroundTime;
                processes[i].waitingTime = sortedProcesses[j].waitingTime;
                processes[i].responseTime = sortedProcesses[j].responseTime;
                break;
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if (jsonMode) {
        ...
        int n;
        std::cout << "=== FCFS CPU Scheduling Simulator ===\n";
